...
const originalFeedUrl = 'https://www.eurogamer.net/feed';
const feed = new Feed({
  title: 'Eurogamer.net RSS Feed',
  description: 'Full article content from Eurogamer.net',
  link: 'https://lukasz-gladek-av.github.io/custom-rss/eurogamer_net_full.xml',
});
...
async function fetchAndProcessFeed() {
  const existingArticles = await loadExistingItems('eurogamer_net_full.xml');
  ...
  if (!hasNewArticles) {
    console.log('No new Eurogamer.net articles found; skipping feed update.');
    return;
  }

  const rssXml = feed.rss2();
  fs.writeFileSync('eurogamer_net_full.xml', rssXml);
  console.log('Successfully updated Eurogamer.net feed with full articles.');
}
