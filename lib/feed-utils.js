...
async function fetchAndParseFeed(url, parser = createRssParser()) {
...
function getPerHostConcurrency(defaultConcurrency = 2) {
  const fromEnv = Number.parseInt(process.env.FETCH_CONCURRENCY_PER_HOST || '', 10);
  if (!Number.isFinite(fromEnv) || fromEnv < 1) {
    return defaultConcurrency;
  }

  return Math.min(fromEnv, 6);
}
...
function parseRetryAfterMs(retryAfterValue) {
  ...
  const asSeconds = Number.parseInt(retryAfterValue, 10);
...
function createHostRequestLimiter(options = {}) {
  const perHostConcurrency = options.perHostConcurrency || 2;
  ...
  async function runQueue(host, state) {
    while (state.activeCount < perHostConcurrency && state.queue.length > 0) {
      const nextJob = state.queue.shift();
      state.activeCount += 1;

      (async () => {
        try {
          const delayMs = Math.max(0, state.throttledUntil - Date.now());
          if (delayMs > 0) {
          ...
          const shouldThrottle = statusCode === 429 || statusCode === 503;

          if (shouldThrottle) {
            state.penaltyLevel = Math.min(state.penaltyLevel + 1, 5);
            const penaltyBackoffMs = Math.min(baseBackoffMs * (2 ** state.penaltyLevel), maxBackoffMs);
            const retryAfterMs = parseRetryAfterMs(error?.response?.headers?.['retry-after']);
            const throttleMs = Math.max(penaltyBackoffMs, retryAfterMs);
            state.throttledUntil = Math.max(state.throttledUntil, Date.now() + throttleMs);
            console.warn(`Throttling host ${host} for ${throttleMs}ms after HTTP ${statusCode}`);
          }
          ...
          if (state.activeCount === 0 && state.queue.length === 0 && state.throttledUntil <= Date.now()) {
            hostStates.delete(host);
            return;
          }

          await runQueue(host, state);
        }
      })();
    }
  }

  return function runWithHostLimit(url, task) {
    const host = getHostFromUrl(url) || '__unknown_host__';
    const state = getHostState(host);

    return new Promise((resolve, reject) => {
      state.queue.push({ task, resolve, reject });
      void runQueue(host, state);
    });
  };
}
